...
export default function HistoryPage() {
  ...
  const past = registrations
    ...
    .sort((a, b) => (b.event?.startAt ? new Date(b.event.startAt).getTime() : 0) - (a.event?.startAt ? new Date(a.event.startAt).getTime() : 0));
  ...
  return (
    <div className="page-content">
      <h1 className="page-title">My Sparrows History</h1>
      <p className="profile-muted" style={{ marginBottom: "1rem" }}>
        Past events you registered for.
      </p>
      {past.length === 0 ? (
        <p className="profile-muted">No past events.</p>
      ) : (
          ...
          {past.map((reg) => (
            <li key={reg.id} className="card-event event-list-item" style={{ padding: "12px", marginBottom: "8px" }}>
              ...
              <div className="event-list-meta">
                {reg.event?.startAt && formatDate(reg.event.startAt)}
              </div>
              <div style={{ marginTop: "8px" }}>
                <span className={`pill ${statusClass(reg.status)}`}>{statusText(reg.status)}</span>
              </div>
              {reg.event?.sportType && (
                <div className="event-list-meta" style={{ marginTop: "4px" }}>Sport: {reg.event.sportType}</div>
              )}
            </li>
          ))}
        </ul>
